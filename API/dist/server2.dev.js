...
animals = ["ant", "bat", "bee", "butterfly", "dog", "horse", "mermaid", "bear", "bird", "camel", "cow", "crab", "crocodile", "dolphin", "dragon", "elephant", "fish", "flamingo", "frog", "giraffe", "hedgehog", "lobster", "monkey", "octopus", "owl", "panda", "parrot", "pig", "rabbit", "raccoon", "rhinoceros", "scorpion", "sea_turtle", "shark", "snail", "spider", "squirrel", "teddy-bear", "zebra", "whale", "tiger", "swan", "snake", "cat", "duck", "lion", "mosquito", "mouse", "sheep", "kangaroo"];
foods = ["apple", "asparagus", "banana", "birthday_cake", "blackberry", "blueberry", "carrot", "coffee_cup", "mushroom", "peas", "pineapple", "pizza", "sandwich", "strawberry", "string_bean", "teapot", "toaster", "watermelon", "wine_bottle", "wine_glass", "toothpaste", "steak", "bread", "broccoli", "cake", "cookie", "donut", "grapes", "hamburger", "ice_cream", "lollipop", "peanut", "pear", "popsicle", "potato", "onion"];
armors = ["bandage", "beard", "belt", "binoculars", "bowtie", "bracelet", "brain", "wristwatch", "underwear", "t-shirt", "goatee", "tooth", "toe", "sweater", "stitches", "stethoscope", "sock", "snorkel", "smiley_face", "sleeping_bag", "skull", "crown", "eyeglasses", "face", "finger", "foot", "hand", "hat", "headphones", "jacket", "lipstick", "moustache", "mouth", "nail", "necklace", "pants", "purse", "shoe", "shorts", "camouflage", "cooler", "ear", "elbow", "eye", "flip_flops", "knee", "leg", "nose", "clock", "diamond", "helmet"];
others = ["alarm_clock", "animal_migration", "backpack", "zigzag", "yoga", "windmill", "waterslide", "washing_machine", "tree", "angel", "The_Mona_Lisa", "traffic_light", "tornado", "The_Eiffel_Tower", "The_Great_Wall_of_China", "tent", "television", "telephone", "table", "swing_set", "sun", "streetlight", "stove", "stereo", "star", "stairs", "spreadsheet", "square", "squiggle", "snowflake", "snowman", "skyscraper", "sink", "barn", "beach", "bed", "bench", "book", "bottlecap", "bridge", "bucket", "bush", "calculator", "calendar", "camera", "castle", "ceiling_fan", "cello", "couch", "cup", "dishwasher", "diving_board", "fence", "fireplace", "floor_lamp", "hexagon", "paper_clip", "picture_frame", "radio", "rain", "remote_control", "saxophone", "see_saw", "hospital", "jail", "keyboard", "light_bulb", "lighthouse", "lightning", "mailbox", "moon", "mountain", "ocean", "octagon", "oven", "palm_tree", "passport", "penguin", "piano", "pillow", "pond", "pool", "postcard", "power_outlet", "rainbow", "river", "roller_coaster", "campfire", "candle", "cell_phone", "chair", "chandelier", "church", "circle", "cloud", "compass", "computer", "door", "dresser", "drums", "dumbbell", "envelope", "eraser", "fan", "feather", "flashlight", "flower", "garden", "grass", "hot_dog", "hot_tub", "hourglass", "house", "house_plant", "hurricane", "key", "ladder", "lantern", "laptop", "leaf", "line", "map", "marker", "matches", "megaphone", "microphone", "microwave", "mug", "paintbrush", "paint_can", "parachute", "pliers"];
all_cat = [vehicles, weapons, animals, foods, armors, others];
all_cat_names = ['vehicles', 'weapons', 'animals', 'foods', 'armors', 'others']; // Load the model.

tfnode.loadLayersModel('file://model.json').then(function (model) {
  var app = express();
  var PORT = 8080;
  app.use(express.json());
  app.post('/image', function (req, res) {
    var b64_image = req.body.b64_image;
    var buffer = Buffer.from(b64_image, 'base64');
    var img = tfnode.node.decodeImage(buffer, 3, 'int32');
    img = tfnode.image.resizeBilinear(img, [28, 28]);
    var n = tfnode.scalar(255.0);
    img = tfnode.cast(img, 'float32');
    var inputs = Array.from(img.dataSync());
    var final_img = [];
    var oneRow = [];

    for (var i = 0; i < 784; i++) {
      var bright = inputs[i * 3]; // rbg

      var onePix = [parseFloat((255 - bright) / 255)]; //Black <-> White
    ...
    var guess = model.predict(tfnode.tensor([final_img]));
    var predictions = Array.from(guess.dataSync()); // Get top K res with index and probability

    var rawProbWIndex = predictions.map(function (probability, index) {
      return {
        index: index,
        probability: probability
      };
    });
    top5 = rawProbWIndex.sort(function (a, b) {
    ...
    top5.map(function (item) {
      ...
      for (var _i = 0; _i < all_cat.length; _i++) {
        var found = all_cat[_i].includes(categories[item.index]);

        if (found) {
          groupName = all_cat_names[_i];
          break;
        }
      }

      list_of_pred.push({
        prediction: categories[item.index],
    ...
    if (!b64_image) {
      res.status(418).send({
        message: "Error, we need image"
      });
    }

    best_pred = list_of_pred[0];
    res.send({
      prediction: "".concat(best_pred.prediction),
      confidence: "".concat(best_pred.confidence),
      group: "".concat(best_pred.group),
      other_pred: list_of_pred
    });
  });
